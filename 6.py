...
def converterHora (h,valor):
    if valor == "A":
        amhora = h%12
    if valor == "P":
        amhora = h+12
    return amhora
def mostraHora (amhora,m):
    print("A hora convertida é: %i:%i %s.M" %(amhora,m,valor))


while True:
    valor = input("Informe o formato que deseja converter: \n'A' se for para A.M\n'P' se for para P.M\nOu 'S' para sair do programa.")
    if valor == "S":
        break
    h = int(input('Informe  apenas a hora: '))
    m = int(input("Informe apenas os minutos: "))
    amhora= (converterHora(h,valor))
    mostraHora(amhora,m)
